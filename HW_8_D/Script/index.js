...
$("#expand").click(function () {
    $.ajax({
        ...
        error: function (xhr, textStatus, errorMessage) {
            alert(errorMessage + "\nI don't know what value should be for expand string");
        }
    });
})
...
$("#search").click(function () {
    $.ajax({
        ...
        success: function (data) {
            alert("Search works correctly in code, but doesn't returns anything for Photos")
            $("#data").empty();
            for (var i = 0; i < data.value.length; i++) {
                display(data.value[i]);
            }
